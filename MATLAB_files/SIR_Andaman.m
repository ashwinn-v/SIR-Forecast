...
% Model Parameters  >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
  cn = 'Andaman and Nicobar';
  
 a = 0.22;
  b = 0.0753;
  f = 13e6;     %scaling factor f=1.20e+07
  
  k0 = 1.15e-9;
  D0 = 5.9e6;    %Constants estimated by the non linear fitting  D0=2.20e+0.05  k0=1.103-0.7
  %Constants estimated by the non linear fitting  D0=2.20e+0.05  k0=1.103-0.7
...
% Starting Date
  zSTART = datenum([2020 2 15]);    %Starting date 14th March 2025
  
  
% Setup  ==============================================================
  S(1) = 0.45;
  t = linspace(0,tMax,num);    %Generating 50000 points within the range of 0 and 500
  h = t(2) - t(1);              %Time step / step height h  xn+1 = x0 + h
  
  %Here value of h=0.1


% SOLVE ODEs with RK4  ===============================================
for n = 1 : num-1                               %Number of integration steps
    
  if n > 1300 && n < 1500; S(n) = 0.45; end 
  if n > 1800 && n < 2000; S(n) = 0.41; end 
 
  if n > 2200; S(n) = 0.10; end    
